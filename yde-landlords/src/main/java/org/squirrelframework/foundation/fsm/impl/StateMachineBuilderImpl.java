...
public class StateMachineBuilderImpl<T extends StateMachine<T, S, E, C>, S, E, C> implements StateMachineBuilder<T, S, E, C> {
    ...
    private final org.squirrelframework.foundation.fsm.impl.ExecutionContext executionContext;
    
    private final List<org.squirrelframework.foundation.fsm.impl.DeferBoundActionInfo<T, S, E, C>> deferBoundActionInfoList = Lists.newArrayList();
    ...
    private StateMachineBuilderImpl(Class<? extends T> stateMachineImplClazz, Class<S> stateClazz, 
        ...
        this.executionContext = new org.squirrelframework.foundation.fsm.impl.ExecutionContext(scriptManager, stateMachineImplClazz, methodCallParamTypes);
    ...
    public DeferBoundActionBuilder<T, S, E, C> transit() {
        checkState();
        return org.squirrelframework.foundation.fsm.impl.FSM.newDeferBoundActionBuilder(deferBoundActionInfoList, executionContext);
    }
    ...
    public ExternalTransitionBuilder<T, S, E, C> externalTransition(int priority) {
        checkState();
        return org.squirrelframework.foundation.fsm.impl.FSM.newExternalTransitionBuilder(states, priority, executionContext);
    }
    ...
    public MultiTransitionBuilder<T, S, E, C> transitions(int priority) {
        checkState();
        return org.squirrelframework.foundation.fsm.impl.FSM.newMultiTransitionBuilder(states, TransitionType.EXTERNAL, priority, executionContext);
    }
    
    @Override
    public LocalTransitionBuilder<T, S, E, C> localTransition(int priority) {
        checkState();
        return org.squirrelframework.foundation.fsm.impl.FSM.newLocalTransitionBuilder(states, priority, executionContext);
    }

    @Override
    public MultiTransitionBuilder<T, S, E, C> localTransitions(int priority) {
        checkState();
        return org.squirrelframework.foundation.fsm.impl.FSM.newMultiTransitionBuilder(states, TransitionType.LOCAL, priority, executionContext);
    }

    @Override
    public InternalTransitionBuilder<T, S, E, C> internalTransition(int priority) {
        checkState();
        return org.squirrelframework.foundation.fsm.impl.FSM.newInternalTransitionBuilder(states, priority, executionContext);
    }
    ...
    private void addStateEntryExitMethodCallAction(String methodName, Class<?>[] parameterTypes, 
        ...
        if(method!=null) {
            ...
            Action<T, S, E, C> methodCallAction = org.squirrelframework.foundation.fsm.impl.FSM.newMethodCallAction(method, weight, executionContext);
            if(isEntryAction) {
    ...
    private void addTransitionMethodCallAction(String methodName, Class<?>[] parameterTypes, 
        ...
        Method method = findMethodCallActionInternal(stateMachineImplClazz, methodName, parameterTypes);
        if(method!=null) {
            Action<T, S, E, C> methodCallAction = org.squirrelframework.foundation.fsm.impl.FSM.newMethodCallAction(method, Action.EXTENSION_WEIGHT, executionContext);
    ...
    private void buildDeferBoundAction(Transit transit) {
        ...
        org.squirrelframework.foundation.fsm.impl.DeferBoundActionInfo<T, S, E, C> deferBoundActionInfo =
                new org.squirrelframework.foundation.fsm.impl.DeferBoundActionInfo<T, S, E, C>(from, to, event);
        if(!transit.callMethod().isEmpty()) {
            Action<T, S, E, C> action = org.squirrelframework.foundation.fsm.impl.FSM.newMethodCallActionProxy(transit.callMethod(), executionContext);
            if(!transit.whenMvel().isEmpty()) {
                final Condition<C> condition = org.squirrelframework.foundation.fsm.impl.FSM.newMvelCondition(transit.whenMvel(), scriptManager);
    ...
    private void buildDeclareTransition(Transit transit) {
            ...
            for(ImmutableTransition<T, S, E, C> t : theFromState.getAllTransitions()) {
                if(t.isMatch(fromState, toState, event, transit.priority(), transit.when(), transit.type())) {
                    ...
                    if(callMethodExpression!=null && callMethodExpression.length()>0) {
                        Action<T, S, E, C> methodCallAction = org.squirrelframework.foundation.fsm.impl.FSM.newMethodCallActionProxy(callMethodExpression, executionContext);
                        mutableTransition.addAction(methodCallAction);
                    }
                    return;
                }
        ...
        if(transit.type()==TransitionType.INTERNAL) {
            InternalTransitionBuilder<T, S, E, C> transitionBuilder =
                    org.squirrelframework.foundation.fsm.impl.FSM.newInternalTransitionBuilder(states, transit.priority(), executionContext);
            ...
            ExternalTransitionBuilder<T, S, E, C> transitionBuilder = (transit.type()==TransitionType.LOCAL) ?
                    org.squirrelframework.foundation.fsm.impl.FSM.newLocalTransitionBuilder(states, transit.priority(), executionContext) :
                        org.squirrelframework.foundation.fsm.impl.FSM.newExternalTransitionBuilder(states, transit.priority(), executionContext);
            From<T, S, E, C> fromBuilder = transitionBuilder.from(fromState);
            boolean isTargetFinal = transit.isTargetFinal() || org.squirrelframework.foundation.fsm.impl.FSM.getState(states, toState).isFinalState();
        ...
        try {
            if(transit.when()!=Conditions.Always.class) {
                ...
                c = org.squirrelframework.foundation.fsm.impl.FSM.newMvelCondition(transit.whenMvel(), scriptManager);
            }
        } catch (Exception e) {
        ...
        if(!Strings.isNullOrEmpty(transit.callMethod())) {
            Action<T, S, E, C> methodCallAction = org.squirrelframework.foundation.fsm.impl.FSM.newMethodCallActionProxy(transit.callMethod(), executionContext);
    ...
    private void buildDeclareState(State state) {
        ...
        if(!Strings.isNullOrEmpty(state.entryCallMethod())) {
            Action<T, S, E, C> methodCallAction = org.squirrelframework.foundation.fsm.impl.FSM.newMethodCallActionProxy(state.entryCallMethod(), executionContext);
            onEntry(stateId).perform(methodCallAction);
        }
        
        if(!Strings.isNullOrEmpty(state.exitCallMethod())) {
            Action<T, S, E, C> methodCallAction = org.squirrelframework.foundation.fsm.impl.FSM.newMethodCallActionProxy(state.exitCallMethod(), executionContext);
    ...
    private void installDeferBoundActions() {
        if(deferBoundActionInfoList.isEmpty()) 
            return;
        for(org.squirrelframework.foundation.fsm.impl.DeferBoundActionInfo<T, S, E, C> deferBoundActionInfo : deferBoundActionInfoList) {
    ...
    private boolean isStateMachineType(Class<?> stateMachineClazz) {
        return stateMachineClazz!= null && org.squirrelframework.foundation.fsm.impl.AbstractStateMachine.class != stateMachineClazz &&
                org.squirrelframework.foundation.fsm.impl.AbstractStateMachine.class.isAssignableFrom(stateMachineClazz);
    }
    ...
    static Method findMethodCallActionInternal(Class<?> target, String methodName, Class<?>[] parameterTypes) {
        return searchMethod(target, org.squirrelframework.foundation.fsm.impl.AbstractStateMachine.class, methodName, parameterTypes);
    }
    ...
    public T newStateMachine(S initialStateId, StateMachineConfiguration configuration, Object... extraParams) {
        ...
        final org.squirrelframework.foundation.fsm.impl.AbstractStateMachine<T, S, E, C> stateMachineImpl = (AbstractStateMachine<T, S, E, C>)stateMachine;
    ...
    public MutableState<T, S, E, C> defineState(S stateId) {
        checkState();
        return org.squirrelframework.foundation.fsm.impl.FSM.getState(states, stateId);
    }
    ...
    public MutableState<T, S, E, C> defineLinkedState(final S stateId, 
        ...
        if(state==null) {
            MutableLinkedState<T, S, E, C> linkedState = org.squirrelframework.foundation.fsm.impl.FSM.newLinkedState(stateId);
    ...
    public MutableState<T, S, E, C> defineTimedState(S stateId,
        ...
        MutableState<T, S, E, C> state = states.get(stateId);
        if(state==null) {
            MutableTimedState<T, S, E, C> timedState = org.squirrelframework.foundation.fsm.impl.FSM.newTimedState(stateId);
    ...
    private void defineChildStatesOn(S parentStateId, StateCompositeType compositeType,
        ...
        if(childStateIds!=null && childStateIds.length>0) {
            MutableState<T, S, E, C> parentState = org.squirrelframework.foundation.fsm.impl.FSM.getState(states, parentStateId);
            ...
            for(int i=0, size=childStateIds.length; i<size; ++i) {
                MutableState<T, S, E, C> childState = org.squirrelframework.foundation.fsm.impl.FSM.getState(states, childStateIds[i]);
    ...
    public EntryExitActionBuilder<T, S, E, C> onEntry(S stateId) {
        checkState();
        MutableState<T, S, E, C> state = org.squirrelframework.foundation.fsm.impl.FSM.getState(states, stateId);
        return org.squirrelframework.foundation.fsm.impl.FSM.newEntryExitActionBuilder(state, true, executionContext);
    }
    ...
    public EntryExitActionBuilder<T, S, E, C> onExit(S stateId) {
        checkState();
        MutableState<T, S, E, C> state = org.squirrelframework.foundation.fsm.impl.FSM.getState(states, stateId);
        return org.squirrelframework.foundation.fsm.impl.FSM.newEntryExitActionBuilder(state, false, executionContext);
    }
