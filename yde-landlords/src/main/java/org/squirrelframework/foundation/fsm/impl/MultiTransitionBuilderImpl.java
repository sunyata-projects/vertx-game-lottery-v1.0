...
class MultiTransitionBuilderImpl<T extends StateMachine<T, S, E, C>, S, E, C> implements
    ...
    MultiTransitionBuilderImpl(Map<S, MutableState<T, S, E, C>> states, TransitionType transitionType,
                               int priority, org.squirrelframework.foundation.fsm.impl.ExecutionContext executionContext) {
        this.states = states;
    ...
    public MultiTo<T, S, E, C> toAmong(S... stateIds) {
        for(S stateId : stateIds) {
            targetStates.add(org.squirrelframework.foundation.fsm.impl.FSM.getState(states, stateId));
        }
        return this;
    }
    ...
    public To<T, S, E, C> toFinal(S stateId) {
        MutableState<T, S, E, C> targetState = org.squirrelframework.foundation.fsm.impl.FSM.getState(states, stateId);
    ...
    public MultiFrom<T, S, E, C> from(S stateId) {
        sourceStates.add(org.squirrelframework.foundation.fsm.impl.FSM.getState(states, stateId));
        return this;
    }
    ...
    public Between<T, S, E, C> between(S fromStateId) {
        sourceStates.add(org.squirrelframework.foundation.fsm.impl.FSM.getState(states, fromStateId));
        return this;
    }
    ...
    public When<T, S, E, C> whenMvel(String expression) {
        for(MutableTransition<T, S, E, C> transition : transitions) {
            Condition<C> cond = org.squirrelframework.foundation.fsm.impl.FSM.newMvelCondition(expression, executionContext.getScriptManager());
    ...
    public void evalMvel(String expression) {
        for(MutableTransition<T, S, E, C> transition : transitions) {
            Action<T, S, E, C> action = org.squirrelframework.foundation.fsm.impl.FSM.newMvelAction(expression, executionContext);
            transition.addAction(action);
        }
    }
    ...
    public void callMethod(String methodName) {
        ...
        for(int i=0, transitionLength=transitions.size(); i<transitionLength; ++i) {
            ...
            Action<T, S, E, C> action = org.squirrelframework.foundation.fsm.impl.FSM.newMethodCallActionProxy(theMethodName, executionContext);
