 ...
 */
@Component(Commands.MissionPlay)
public class MissionPlayCommandHandler extends AbstractGameControllerCommandHandler {
    class CommandErrorCode {

    }

    Logger logger = LoggerFactory.getLogger(MissionPlayCommandHandler.class);
    ...
    public void execute(OctopusRequest request, OctopusResponse response) throws Exception {
        try {
            if (!this.canAccpet(request, GameMissionEvent.Play)) {
            ...
            MissionPlayPhaseModel playPhaseModel = gameModel.getLastPlayPhaseModel();
            MissionPlayPhaseData phaseData = playPhaseModel.getPhaseData();
//            if (phaseData.getNextPosition() != position) {
//                throw new InvalidRoleException("角色位置错乱");
//            }
            MissionPlayPhasePlayDataItem lastItem = phaseData.getLastPlayDataItem();
            ...
            if (lastItem == null) {//此次为第一次出牌
                ...
                item.setCenterCards(new ArrayList<>(dealPhaseData.getCenterCards()))
                        .setRightCards(new ArrayList<>(dealPhaseData.getRightCards()))
                        .setLeftCards(new ArrayList<>(dealPhaseData.getLeftCards()));
                item.getCenterCards().addAll(dealPhaseData.getUnderCards());
...
//            }
            ...
            gameController.fire(GameMissionEvent.Play, gameModel);
    ...
    public void play(MissionPlayPhaseDataItem item, MissionPlayPhaseData phaseData) throws TException,
...
//            }
    ...
    public boolean syncCards(MissionPlayPhaseData phaseData, MissionPlayPhaseDataItem item, List<Integer> cardList,
                             Integer placeRole) {
...
//    public MissionPlayPhaseDataItem generatorPlayDataItem(GameMissionModel gameModel) {
//
//        return item;
//    }
