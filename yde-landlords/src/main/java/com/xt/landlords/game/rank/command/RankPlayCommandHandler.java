...
import com.xt.landlords.game.rank.phase.RankPlayPhaseDataItem;
import com.xt.landlords.game.rank.phase.RankPlayPhaseModel;
import com.xt.landlords.game.rank.phase.RankPlayPhasePlayDataItem;
 ...
 */
@Component(Commands.RankPlay)
public class RankPlayCommandHandler extends AbstractGameControllerCommandHandler {
    class CommandErrorCode {

    }

    Logger logger = LoggerFactory.getLogger(RankPlayCommandHandler.class);
    ...
    public void execute(OctopusRequest request, OctopusResponse response) throws Exception {
        try {
            if (!this.canAccpet(request, GameRankEvent.Play)) {
            ...
            GameRankModel gameModel = (GameRankModel) gameController.getGameModel();
            ...
            RankPlayPhaseModel playPhaseModel = gameModel.getLastPlayPhaseModel();
            if(playPhaseModel==null){
                playPhaseModel = gameModel.addPlayPhase();
            }
            RankPlayPhaseData phaseData = playPhaseModel.getPhaseData();
//            if (phaseData.getNextPosition() != position) {
//                throw new InvalidRoleException("角色位置错乱");
//            }
            RankPlayPhasePlayDataItem lastItem = phaseData.getLastPlayDataItem();
            RankPlayPhasePlayDataItem item = new RankPlayPhasePlayDataItem();

            if (lastItem == null) {//此次为第一次出牌
                DealPhaseModel phase = (DealPhaseModel) gameModel.getLastDealPhaseModel();
                DealPhaseData dealPhaseData = (DealPhaseData) phase.getPhaseData();
//                RankPlayPhaseDealDataItem dealPhaseData = playPhaseData.get.getDealDataItem();
...
//            }
            phaseData.addPlayDataItem(item);
            gameController.fire(GameRankEvent.Play, gameModel);
    ...
    public void play(RankPlayPhaseDataItem item, RankPlayPhaseData phaseData) throws TException,
...
//            }
    ...
    public boolean syncCards(RankPlayPhaseData phaseData, RankPlayPhaseDataItem item, List<Integer> cardList,
                             Integer placeRole) {
...
//    public RankPlayPhaseDataItem generatorPlayDataItem(GameRankModel gameModel) {
//
//        return item;
//    }
