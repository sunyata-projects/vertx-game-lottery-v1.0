 ...
 */
public final class WebSocketPuzzleClient {
...
//                    ch.closeFuture().sync();
                    ...
                    GamePuzzle.PuzzleDealRequestMsg.Builder builder = GamePuzzle.PuzzleDealRequestMsg.newBuilder();
                    GamePuzzle.PuzzleDealRequestMsg puzzleDealRequestMsg = builder.setGameType(10003).build();
                    ByteBuf buffer = Unpooled.buffer();
                    byte[] bytes = puzzleDealRequestMsg.toByteArray();
                    buffer.writeInt(52003).writeInt(2323232).writeFloat(1.0f).writeInt(bytes.length).writeBytes(bytes);
                    WebSocketFrame frame = new BinaryWebSocketFrame(buffer);
                    ch.writeAndFlush(frame);
                    ...
                    GamePuzzle.ClearGameRequestMsg.Builder builder = GamePuzzle.ClearGameRequestMsg.newBuilder();
                    GamePuzzle.ClearGameRequestMsg puzzleClearRequestMsg = builder.build();
                    ByteBuf buffer = Unpooled.buffer();
                    byte[] bytes = puzzleClearRequestMsg.toByteArray();
