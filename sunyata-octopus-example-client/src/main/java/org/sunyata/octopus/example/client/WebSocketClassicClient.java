...
//                    ch.closeFuture().sync();
                    ...
                    Common.BetRequestMsg betRequestMsg = builder.setGameType(10008).setAmt(100).build();
                    ByteBuf buffer = Unpooled.buffer();
                    ...
                    Common.DealRequestMsg dealRequestMsg = builder.setGameType(10008).build();
                    ByteBuf buffer = Unpooled.buffer();
                    byte[] bytes = dealRequestMsg.toByteArray();
                    buffer.writeInt(55002).writeInt(2323232).writeFloat(1.0f).writeInt(bytes.length).writeBytes(bytes);
                    WebSocketFrame frame = new BinaryWebSocketFrame(buffer);
                    ch.writeAndFlush(frame);
                ...
                } else if ("turn".equals(msg.toLowerCase())) {

                    Common.TurntableRequestMsg.Builder builder = Common.TurntableRequestMsg.newBuilder();

                    Common.TurntableRequestMsg turntableRequestMsg = builder.build();
                    ByteBuf buffer = Unpooled.buffer();
                    byte[] bytes = turntableRequestMsg.toByteArray();
                    buffer.writeInt(55004).writeInt(2323232).writeFloat(1.0f).writeInt(bytes.length).writeBytes(bytes);
                    WebSocketFrame frame = new BinaryWebSocketFrame(buffer);
                    ch.writeAndFlush(frame);
                } else if ("summary".equals(msg.toLowerCase())) {
                    GameClassic.SummaryClearGameRequestMsg.Builder builder = GameClassic.SummaryClearGameRequestMsg.newBuilder();
                    GameClassic.SummaryClearGameRequestMsg summaryClearGameRequestMsg = builder.build();
                    ByteBuf buffer = Unpooled.buffer();
                    byte[] bytes = summaryClearGameRequestMsg.toByteArray();
                    buffer.writeInt(55007).writeInt(2323232).writeFloat(1.0f).writeInt(bytes.length).writeBytes(bytes);
                    WebSocketFrame frame = new BinaryWebSocketFrame(buffer);
                    ch.writeAndFlush(frame);
                    ...
                    buffer.writeInt(55005).writeInt(2323232).writeFloat(1.0f).writeInt(bytes.length).writeBytes(bytes);
                    WebSocketFrame frame = new BinaryWebSocketFrame(buffer);
                    ch.writeAndFlush(frame);
                    ...
                    buffer.writeInt(55006).writeInt(2323232).writeFloat(1.0f).writeInt(bytes.length).writeBytes(bytes);
                    WebSocketFrame frame = new BinaryWebSocketFrame(buffer);
                    ch.writeAndFlush(frame);
                    ...
                    buffer.writeInt(55008).writeInt(2323232).writeFloat(1.0f).writeInt(bytes.length).writeBytes(bytes);
                    WebSocketFrame frame = new BinaryWebSocketFrame(buffer);
                    ch.writeAndFlush(frame);
