 ...
 */
public final class WebSocketMissionClient {
...
//                    ch.closeFuture().sync();
                    ...
                    Common.BetRequestMsg betRequestMsg = builder.setGameType(10005).setAmt(100).build();
                    ByteBuf buffer = Unpooled.buffer();
                    ...
                    Common.DealRequestMsg dealRequestMsg = builder.setGameType(10005).build();
                    ByteBuf buffer = Unpooled.buffer();
                    byte[] bytes = dealRequestMsg.toByteArray();
                    buffer.writeInt(51002).writeInt(2323232).writeFloat(1.0f).writeInt(bytes.length).writeBytes(bytes);
                    WebSocketFrame frame = new BinaryWebSocketFrame(buffer);
                    ch.writeAndFlush(frame);
                    ...
                    for (int i = 0; i < 10; i++) {
                        ...
                        buffer.writeInt(51003).writeInt(2323232).writeFloat(1.0f).writeInt(bytes.length).writeBytes
                                (bytes);
                    ...
                    buffer.writeInt(51004).writeInt(2323232).writeFloat(1.0f).writeInt(bytes.length).writeBytes(bytes);
                    WebSocketFrame frame = new BinaryWebSocketFrame(buffer);
                    ch.writeAndFlush(frame);
